  ...
  return (
    <header
      className={`sticky top-0 z-50 bg-white transition-all duration-500 ${
        scrolled ? "shadow-[0_8px_30px_rgba(27,44,74,0.12)] border-b border-transparent" : "border-b border-border shadow-none"
      }`}
    >
        ...
        <nav className="hidden md:flex items-center gap-7" role="navigation" aria-label="Menu principal">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="relative text-[13px] font-medium text-foreground/75 hover:text-primary transition-colors duration-300
                         after:absolute after:bottom-[-2px] after:left-0 after:h-[2px] after:w-0 after:bg-accent
                         after:transition-all after:duration-500 hover:after:w-full after:ease-in-out"
            >
              {item.label}
            </a>
          ))}
